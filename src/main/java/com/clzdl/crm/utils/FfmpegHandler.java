...
public class FfmpegHandler {
	...
	public void grabPicture2Gif(int begSec, int endSec, String gifFile, Integer gifDelayTime, Integer frequency)
		...
		try {
			...
			encoderGif.setDelay(gifDelayTime);
			encoderGif.start(gifFile);

			if (begSec > 0) {
				begSec += videoStream.start_time();
				int retSeek = av_seek_frame(fmtCtx, videoStreamIndex, (long) (begSec / av_q2d(videoStream.time_base())),
						AVSEEK_FLAG_ANY);
				if (retSeek < 0) {
			...
			while (av_read_frame(fmtCtx, pkt) >= 0) {
				if (pkt.stream_index() == videoStreamIndex) {
					...
					if (ret2 >= 0) {
						...
						encoderGif.addFrame(toImage(pFrameRGB, codecCtx.width(), codecCtx.height()));
						if (pkt.pts() > endSecBaseStream) {
							break;
						}
					}
				}
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			handler.grabPicture2Gif(120, 126, "/home/java/bbbbbbb.gif", 40, 5);
		} finally {
			handler.free();
		}
	}
